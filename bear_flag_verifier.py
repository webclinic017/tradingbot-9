class BearFlagVerifier:

    def verifyBearFlag(self, entry_candle_infos, transaction):
        for entry_candle_info in entry_candle_infos:
            try:
                ...
                for time, currentCandle in subset.iterrows():
                    if currentCandle[stock]['low'] < first_target:
                        money_made = (entry - first_target)
                        transaction.add(money_made)
                        print("True Prediction", "Money Made", money_made, transaction.totalMoneyMade)
                        break;
                    if currentCandle[stock]['high'] > stoploss:
                        money_lost = stoploss - entry;
